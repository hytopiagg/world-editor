...
export class SpatialHashGrid {
	...
	initializeFromBinary(data) {
		const { blockIds, coordinates, hashTable, collisionTable, size, hashConstants } = data;
		
		// Validate the incoming data
		if (!this.validateGridData(blockIds, coordinates, hashTable, collisionTable, size)) {
			console.error("SpatialHashGrid: Invalid binary data received from worker, falling back to empty grid");
			this.reset();
			return;
		}
	...
	validateGridData(blocks, coords, hashTable, collisionTable, size) {
		...
		if (blocks.length < size || 
			...
			console.error("SpatialHashGrid: Arrays are too small for specified size");
			return false;
		}
		...
		if (size > 0) {
			for (let i = 0; i < Math.min(3, size); i++) {
				if (blocks[i] === undefined || 
					...
					console.error(`SpatialHashGrid: Undefined values found in data at index ${i}`);
					return false;
				}
			}
		}
	...
	logSampleBlocks(count = 5) {
		...
		console.log(`SpatialHashGrid: Sample of ${sampleCount} blocks from grid:`);
	...
	hash(x, y, z) {
		...
		return ((x * this.HASH_PRIME1) ^ 
				(y * this.HASH_PRIME2) ^ 
				(z * this.HASH_PRIME3)) >>> 0; // Force unsigned 32-bit
	}
	...
	set(x, y, z, blockId) {
		...
		if (typeof x === 'string') {
			...
			if (typeof y === 'number' && z === undefined) {
				...
				console.warn('Invalid parameters for set method', { x, y, z, blockId });
				return false;
			}
		}
		...
		if (x === undefined || y === undefined || z === undefined ||
			...
			console.warn('Invalid coordinates in set method', { x, y, z, blockId });
			return false;
		}
	...
	remove(posKey, y, z) {
		...
		if (isNaN(x) || isNaN(y) || isNaN(z)) {
			console.warn(`SpatialHashGrid: Invalid coordinates in remove(${x},${y},${z})`);
			return false;
		}
