...
const ImportStep = ({ worldData, blockMappings, onImportComplete }) => {
    ...
    const regionWidth = selectedRegion
        ...
        : 0;
    const regionHeight = selectedRegion
        ? selectedRegion.maxY - selectedRegion.minY + 1
        : 0;
    const regionDepth = selectedRegion
                    ...
                    customMappings.map(async (mapping) => {
                        ...
                        if (mapping.textureUri) {
                            ...
                            if (!existingBlock) {
                                ...
                                console.log(
                                    `Custom texture already exists for ${mapping.blockType}`
                                );
                            }
                        }
                    })
    ...
    return (
        <div className="import-step">
            ...
            {importing && (
                <div className="import-progress">
                    ...
                    <p className="hint-text">
                        This may take a few moments for large maps...
                    </p>
                </div>
            )}
            <div className="import-summary">
                ...
                <p>
                    Block types to import: {mappedBlockTypes} (out of{" "}
                    {blockTypeCount})
                </p>
                <p>
                    Estimated blocks to process:{" "}
                ...
                <div className="import-result">
                    <div className="success-message">
                        ...
                        <p>
                            ...
                            {result.stats && result.stats.processedBlocks
                                ? result.stats.processedBlocks.toLocaleString()
                                : 0}{" "}
                            blocks.
                        </p>
                    </div>
                    <div className="stats-container">
                        ...
                        <div className="stats-item">
                            ...
                            <span>
                                {result.stats && result.stats.skippedBlocks
                                    ? result.stats.skippedBlocks.toLocaleString()
                                    : 0}
                            </span>
                        </div>
                        <div className="stats-item">
                            ...
                            <span>
                                {result.stats && result.stats.uniqueBlockTypes
                                    ? result.stats.uniqueBlockTypes.length
                                    : 0}
                            </span>
                        </div>
                        {result.stats && result.stats.originalCenter && (
                            <div className="stats-item">
                                ...
                                <span>
                                    ({result.stats.originalCenter.x},{" "}
                                    {result.stats.originalCenter.y},{" "}
                                    {result.stats.originalCenter.z})
                                </span>
                            </div>
                        )}
                    </div>
                    <div className="world-bounds-info">
                        ...
                        <p>
                            The map has been centered at (0,0) on the X-Z plane
                            with the bottom at Y=0.
                        </p>
                        {result.stats && result.stats.worldBounds && (
                            <div className="bounds-diagram">
                                <p>
                                    Map now extends from (
                                    {result.stats.worldBounds.minX},{" "}
                                    {result.stats.worldBounds.minY},{" "}
                                    {result.stats.worldBounds.minZ}) to (
                                    {result.stats.worldBounds.maxX},{" "}
                                    {result.stats.worldBounds.maxY},{" "}
                                    {result.stats.worldBounds.maxZ})
                                </p>
                                <div className="bounds-visual">
                                    <div
                                        className="bounds-box"
                                        style={{
                                            width: "100%",
                                            height: "100px",
                                            position: "relative",
                        ...
                        <p>
                            Click "Complete Import" to add these blocks to your
                            HYTOPIA world.
                        </p>
                    </div>
                </div>
