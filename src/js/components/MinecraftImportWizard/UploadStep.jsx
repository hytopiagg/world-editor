...
const createWorker = () => {
    return new Worker(
        new URL("../../workers/minecraftParserWorker.js", import.meta.url)
    );
};
...
export const getCurrentBlocksData = () =>
    globalWorldDataStorage.currentBlocksData;
    ...
    const handleFileSelect = useCallback(
        async (event) => {
            ...
            if (!file.name.endsWith(".zip")) {
                setError(
                    "Please upload a Minecraft world as a ZIP file. The file should have a .zip extension."
                );
                return;
            }
            if (file.size > 100 * 1024 * 1024) {
                setError(
                    "The world file is too large (>100MB). For better performance, consider selecting a smaller world or exporting a specific region."
                );
                return;
            }
                ...
                workerRef.current.onmessage = (e) => {
                    ...
                    if (type === "progress") {
                        ...
                        if (data && data.bounds) {
                            ...
                            console.warn(
                                "World size data missing bounds, using defaults"
                            );
                            setSelectedBounds({
                                minX: -150,
                                maxX: 149,
                        ...
                        console.log(
                            `[CHUNKS] Received chunk ${data.chunkId} of ${data.totalChunks} (${Object.keys(data.blocks).length} blocks)`
                        );

                        globalWorldDataStorage.chunkedBlocksData[data.chunkId] =
                            data.blocks;
                        ...
                        const percentComplete = Math.floor(
                            (data.chunkId / data.totalChunks) * 100
                        );
                        setProgressMessage(
                            `Processing block data: ${percentComplete}% (Chunk ${data.chunkId}/${data.totalChunks})`
                        );
                    } else if (type === "worldParsed") {

                        console.log(
                            "[TIMING] UploadStep: worldParsed event received, all chunks complete"
                        );
                        setUploadingWithNotify(false);
                        setProgress(100);
                        setProgressMessage(
                            "World loading complete! Click Next to continue."
                        );
                        setShowSizeSelectorWithNotify(false); // Hide region selector after parsing

                        console.log(
                            "[CHUNKS] Combining all chunks into one blocks object"
                        );
                        ...
                        const processChunks = async () => {
                            try {
                                const totalChunks =
                                    globalWorldDataStorage.totalChunks;
                                ...
                                for (
                                    ...
                                    const progress = Math.floor(
                                        (processedChunks / totalChunks) * 80
                                    );

                                    loadingManager.updateLoading(
                                        `Combining chunks: ${processedChunks}/${totalChunks}`,
                                        progress
                                    );
                                ...
                                console.log(
                                    `[CHUNKS] Combined ${Object.keys(combinedBlocks).length} blocks from ${totalChunks} chunks`
                                );
                                globalWorldDataStorage.currentBlocksData =
                                    combinedBlocks;
                                ...
                                if (!selectedBounds) {
                                    console.warn(
                                        "No bounds selected, using defaults from world size info"
                                    );
                                ...
                                const worldDataWithRegion = {
                                    ...data,
                                    blocksCount:
                                        Object.keys(combinedBlocks).length,
                                ...
                                console.log(
                                    "[TIMING] UploadStep: About to call onWorldLoaded with complete worldData"
                                );
                    ...
                    {worldData &&
                        ...
                        progress === 100 && (
                            <div className="version-warning-message">
                                <p>
                                    <strong>Warning:</strong> This world is from
                                    an older version of Minecraft (Data Version{" "}
                                    {worldData.worldVersion}).
                                </p>
                                <p>
                                    For best results, please update your world
                                    to Minecraft 1.21 (Data Version 3953) before
                                    importing.
                                </p>
                                <p>
                                ...
                                <p>
                                    You cannot proceed with the import until you
                                    update your world to Minecraft 1.21.
                                </p>
                            </div>
                        )}
                </div>
                ...
                <div className="progress-details">
                    {chunkProgress.total > 0 && (
                        <div className="chunk-progress">
                            <p>
                                <strong>Data Chunks:</strong>{" "}
                                {chunkProgress.received} of{" "}
                                {chunkProgress.total} received (
                                {Math.round(
                                    (chunkProgress.received /
                                        chunkProgress.total) *
                                        100
                                )}
                                %)
                    ...
                    {memoryUsage && (
                        <div className="memory-usage">
                            <p>
                                ...
                                {Math.round(memoryUsage.used / (1024 * 1024))}{" "}
                                MB /{" "}
                                {Math.round(memoryUsage.limit / (1024 * 1024))}{" "}
                                MB
                            </p>
                            ...
                            {memoryUsage.used > memoryUsage.limit * 0.8 && (
                                <p className="memory-tip">
                                    Memory usage is high - try reducing the
                                    region size or filtering more blocks.
                                </p>
                            )}
                        </div>
                    )}
                        ...
                        <div className="filter-stats">
                            ...
                            {filterStats.totalSkipped > 0 && (
                                <p className="filter-tip">
                                    ...
                                    {filterStats.totalSkipped.toLocaleString()}{" "}
                                    blocks (
                                    {Math.round(
                                        (filterStats.totalSkipped /
                                            (filterStats.totalBlocks || 1)) *
                                            100
                                    )}
                                    % of total)
                                </p>
        ...
        <div className="upload-step">
            ...
            {!uploading && !showSizeSelector && !worldData && (
                <>
                    ...
                    <p className="step-description">
                        Import a Minecraft world by uploading a ZIP file of your
                        world folder. You'll be able to select which region to
            ...
            {!uploading && !showSizeSelector && !worldData && (
                <div className="compatibility-info">
                    ...
                    <p>
                        <strong>Supported Versions:</strong> Minecraft Java
                        Edition 1.21.x or newer
                    </p>
                    <p>
                        <strong>Maximum Import Size:</strong> 500 x 500 blocks
                        (X/Z dimensions)
                    </p>
                    <p>
                        <strong>Maximum Block Limit:</strong>{" "}
                        {options.maxBlocks.toLocaleString()} blocks (can be
                        adjusted in advanced options)
                    </p>
                    <div className="export-instructions">
                        <h4>How to Export Your Minecraft World:</h4>
                        <ol>
                            <li>
                                Open Minecraft Java Edition and select the world
                                you want to export
                            </li>
                            <li>
                                Click "Edit" then "Export" to create a backup of
                                your world
                            </li>
                            <li>
                                Locate the exported ZIP file (usually in your
                                "saves" folder)
                            </li>
                            <li>
                                Upload the ZIP file using the drag & drop area
                                above
                            </li>
                        </ol>
                        <p className="note">
                            Note: For large worlds, consider using the advanced
                            options to limit the region size or reduce memory
                            usage.
                        </p>
                    </div>
                </div>
                ...
                <>
                    <div className="world-stats">
                        <p>
                            ...
                            {worldSizeInfo.worldFolder || "Unknown"}
                        </p>
                        <p>
                            <strong>Size:</strong> {worldSizeInfo.size.width} x{" "}
                            {worldSizeInfo.size.height} x{" "}
                            {worldSizeInfo.size.depth} blocks
                        </p>
                        <p>
                            <strong>Regions:</strong>{" "}
                            {worldSizeInfo.size.regionCount} (
                            {worldSizeInfo.size.regionWidth}x
                            {worldSizeInfo.size.regionDepth})
                        </p>
                        <p>
                            <strong>Estimated Size:</strong>{" "}
                            {worldSizeInfo.size.approximateSizeMB} MB
                        </p>
                        <p>
                            <strong>Maximum Import Size:</strong> 500 x 500
                            blocks (X/Z dimensions)
                        </p>
                        ...
                        {worldSizeInfo.worldVersion && (
                            <p>
                                ...
                                {worldSizeInfo.worldVersion > 3953 && (
                                    <span className="version-compatible">
                                        {" "}
                                        (Newer than Minecraft 1.21)
                                    </span>
                                )}
                                {worldSizeInfo.worldVersion < 3953 && (
                                    <span className="version-older">
                                        {" "}
                                        (Older than Minecraft 1.21 - Needs
                                        updating)
                                    </span>
                                )}
                            </p>
                        )}
                        {worldSizeInfo.size.width * worldSizeInfo.size.depth >
                            5000 * 5000 && (
                            <div className="warning-box">
                                <p>
                                    <strong>Warning:</strong> This is a very
                                    large world. Importing the entire map may
                                    cause performance issues.
                                </p>
                                <p>
                                    It's recommended to select a smaller region
                                    to import.
                                </p>
                            </div>
                        )}
                        ...
                        {worldSizeInfo.worldVersion &&
                            worldSizeInfo.worldVersion < 3953 && (
                                <div className="version-warning-box">
                                    <p>
                                        <strong>Warning:</strong> This world is
                                        from an older version of Minecraft (Data
                                        Version {worldSizeInfo.worldVersion}).
                                    </p>
                                    <p>
                                        Please update your world to Minecraft
                                        1.21 (Data Version 3953) before
                                        importing.
                                    </p>
                                </div>
                            )}
                    ...
                    <div className="world-map-container">
                        ...
                        <div className="bounds-inputs">
                            <div className="bounds-group">
                                <label>
                                    X Bounds (Min: {worldSizeInfo.bounds.minX},
                                    Max: {worldSizeInfo.bounds.maxX}):
                                </label>
                            ...
                            <div className="bounds-group">
                                <label>
                                    Z Bounds (Min: {worldSizeInfo.bounds.minZ},
                                    Max: {worldSizeInfo.bounds.maxZ}):
                                </label>
                                <div className="bounds-input-row">
                                    <input
                                        type="number"
                                ...
                                (selectedBounds.maxX -
                                    ...
                                    <div className="warning-box">
                                        <p>
                                            ...
                                            selected area exceeds the
                                            recommended maximum size of 500 x
                                            500 blocks.
                                        </p>
                                        <p>
                                            This may cause performance issues or
                                            fail to import. Consider selecting a
                                            smaller area.
                                        </p>
                                    </div>
                                )}
                            ...
                            {showAdvanced && (
                                <div className="options-panel">
                                    ...
                                    <p className="optimization-warning">
                                        For larger worlds, adjust these settings
                                    ...
                                    <div className="option-row">
                                        <label>
                                            <input
                                                type="checkbox"
                                                checked={
                                                    options.excludeTransparentBlocks
                                                }
                                                onChange={(e) =>
                                                    handleOptionChange(
                                                        "excludeTransparentBlocks",
                                                        e.target.checked
                                                    )
                                                }
                                            />
                                    ...
                                    <div className="option-row">
                                        <label>
                                            ...
                                            />
                                            Limit regions (load only central
                                            area)
                                        </label>
                                    </div>
                                        ...
                                        <div className="option-col">
                                            ...
                                            <label>
                                                ...
                                                >
                                                    ...
                                                    <option value="2">
                                                        Load every other chunk
                                                        (1:2)
                                                    </option>
                                                    <option value="4">
                                                        Load every 4th chunk
                                                        (1:4)
                                                    </option>
                                                    <option value="8">
                                                        Load every 8th chunk
                                                        (1:8)
                                                    </option>
                                                </select>
                                            </label>
            ...
            {worldData && !uploading && !showSizeSelector && (
                <div className="success-message">
                    ...
                    <p className="next-step-info">
                        Advancing to block mapping step...
                    </p>
                    {worldData.blocksCount && (
                        <div className="import-summary">
                            <p>
                                <strong>Imported Blocks:</strong>{" "}
                                {worldData.blocksCount.toLocaleString()} blocks
                            </p>
                            <p>
                                <strong>Selected Area:</strong>{" "}
                                {worldData.selectedRegion.width} x{" "}
                                {worldData.selectedRegion.height} x{" "}
                                {worldData.selectedRegion.depth} blocks
                            </p>
                        </div>
                    )}
                </div>
