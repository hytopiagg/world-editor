...
function hash(x, y, z) {
	...
	return ((x * HASH_PRIME1) ^ 
			(y * HASH_PRIME2) ^ 
			(z * HASH_PRIME3)) >>> 0; // Force unsigned 32-bit integer
}

// Handle messages from the main thread
self.onmessage = function(event) {
  const { operation, blocks, chunkSize, added, removed, current } = event.data;
  ...
  try {
    ...
    if (operation === 'buildGrid') {
      console.log(`[Worker] Starting buildGrid operation with ${blocks?.length || 0} blocks`);
      ...
      console.log(`[Worker] Sent grid data back to main thread with ${result.size} blocks`);
...
function buildSpatialGrid(blocks, chunkSize) {
  ...
  console.log(`[Worker] Building spatial grid with ${blocks.length} blocks and chunk size ${chunkSize}`);
  ...
  try {
    ...
    console.log(`[Worker] Grid built in ${processTime.toFixed(2)}s with ${blockCount} blocks`);
    console.log(`[Worker] Stats: ${validBlocksProcessed} valid, ${airBlocksSkipped} air skipped, ${skippedInvalidKeys} invalid keys`);
    
    // Create spatial index data
    const spatialIndex = buildSpatialIndex(actualCoordinates, blockCount, chunkSize);
...
function buildSpatialIndex(coordinates, size, chunkSize) {
  ...
  console.log(`Worker: Built spatial index with ${chunkCount} chunks, avg ${avgBlocksPerChunk.toFixed(1)} blocks/chunk`);
